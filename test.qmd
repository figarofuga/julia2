...
"""
    stepify(df::DataFrame; xcol::Symbol=:x, ycol::Symbol=:y)

`df` から x, y データを取得し、ステッププロットに適した形に加工した新たな `DataFrame` を返す。
- `xcol`, `ycol` で列名を指定（デフォルトでは `:x`, `:y`）
- 返される DataFrame は、ステップ状に補完された (x, y) の座標リストとなる
"""
function stepify(df::DataFrame; xcol::Symbol=:x, ycol::Symbol=:y)
    # x でソート（既にソート済みなら不要）
    df_sorted = sort(df, xcol)
    xvals = collect(df_sorted[!, xcol])
    yvals = collect(df_sorted[!, ycol])
    ...
    for i in 1:(length(xvals)-1)
        # 区間 [xᵢ, xᵢ₊₁) の横方向のステップ
        push!(newx, xvals[i+1])
        push!(newy, yvals[i])   # y は変化しない(横線)
        
        # xᵢ₊₁ の位置で縦方向に y が変化
        push!(newx, xvals[i+1])
        push!(newy, yvals[i+1]) # y を次の値に
    end
    
    # 結果を DataFrame にして返す
    return DataFrame(x=newx, y=newy)
end

stepify(res, xcol = :survtime, ycol = :lower)

stepify(res, xcol = :survtime, ycol = :higher)
...
stairs!(ax1, expanded_dfs[1][!, :survtime], expanded_dfs[1][!, :survival]; step=:post, color = :blue)
stairs!(ax1, expanded_dfs[2][!, :survtime], expanded_dfs[2][!, :survival]; step=:post, color = :red)

stairs!(ax1, expanded_dfs[1][!, :survtime], expanded_dfs[1][!, :lower]; step=:post, color = (:blue, 0.2))
stairs!(ax1, expanded_dfs[1][!, :survtime], expanded_dfs[1][!, :higher]; step=:post, color = (:blue, 0.2))

band!(ax1, expanded_dfs[1][!, :survtime] , expanded_dfs[1][!, :higher], expanded_dfs[1][!, :lower]; color = (:blue, 0.2))
