...
dat2 = read_csv("rawdata/pbcseq.csv", missingstring = "NA")

dat_glm = @chain dat2 begin
    TidierData.@filter((status == 0) || (status == 1))
    TidierData.@mutate(chol2 = is_float(chol))
end
...
gm1 = fit(GeneralizedLinearModel, @formula(status ~ age + sex + trt + ascites + hepato + albumin), dat_glm, Binomial())
