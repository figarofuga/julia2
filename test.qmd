...
stepify(expanded_dfs[1])

stepify(expanded_dfs[2])
...
text!(ax2, repeat(set_times, inner=2), repeat(1:2, outer = length(set_times)), text=string.(res[:, :natrisk]), align = (:center, :center))
rowsize!(f.layout, 2, Relative(1/3))
...
f = Figure(padding=(30, 30, 30, 30))  # 全体のパディングを増やす
...
ax2 = Axis(f[2, 1], title="Number at risk", titlealign=:left, xticks=collect(0:1000:5500),xlabel="",
           ...
           yticks = [1.3, 1.7],
           ytickformat=values -> [value == 1.3 ? "m" : value == 1.7 ? "f" : "" for value in values],
           limits = (nothing, nothing, 1, 2)
           ) 
...
# テキストの位置を調整（Y座標を1.5と1.8に変更）
text!(ax2, repeat(set_times, inner=2), repeat([1.3, 1.7], outer=length(set_times)), 
      text=string.(res[:, :natrisk]), align=(:center, :center))

# パネル間の余白を設定
rowgap!(f.layout, 15) # パネル間に15ピクセルの間隔を追加
rowsize!(f.layout, 2, Relative(1/4)) # Number at riskパネルの相対サイズを調整
